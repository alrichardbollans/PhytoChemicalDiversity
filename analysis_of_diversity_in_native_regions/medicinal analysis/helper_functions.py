...
def get_working_data_for_medicinal_species_analysis():
    phy_div_data = pd.read_csv(
        os.path.join('..','..', 'collect_and_compile_data', 'get_phylogenetic_diversities', 'outputs', 'group_data', 'native_regions_medicinal_species_transformed.csv'),
        index_col=0)
    trait_data = pd.read_csv(
        os.path.join('..', '..','collect_and_compile_data', 'get_diversity_metrics', 'outputs', 'group_data', 'native_regions_medicinal_species_transformed.csv'),
        index_col=0)
