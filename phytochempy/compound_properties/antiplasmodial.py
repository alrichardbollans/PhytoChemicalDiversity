...
def add_chembl_data_to_compound_df(compound_df: pd.DataFrame, assay_csv: str, output_csv: str, pchembl_active_threshold: float = 6,
    ...
    def summarise_chembl(df):
        sorted = df[
            ['Compound_Name', 'InChIKey', 'InChIKey_simp', 'mean_ic50_μM_by_InChIKey_simp', 'assay_pchembl_value', 'natural_product']].sort_values(
            by='assay_pchembl_value',
            ascending=False).drop_duplicates(
            subset=[compound_id_col], keep='first')
        return sorted
...
if __name__ == '__main__':
    ##Example usage
    get_compound_info_from_chembl_apm_assays('assays.csv')
